...
extern uint16 f;
extern uint8 State_data[3];//={'0','0','0'};
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
/**************************我们的.c文件里的函数*********************************/
extern void Absence_check(uint8 x);//狗清定义的节点缺席检测函数,有x次签到不到就报警。
extern void BT2Z_transmit(uint8 *group);//狗清定义的蓝牙到zigbee信息搬运函数
extern void SerialCMD(mtOSALSerialData_t *cmdMsg);//
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case KEY_CHANGE:
          ...
          break;
        case CMD_SERIAL_MSG: //由 串口收到数据后由 T MT_UART  层传递过来的义 数据，用网蜂方法接收，编译时不定义  MT相关内容
