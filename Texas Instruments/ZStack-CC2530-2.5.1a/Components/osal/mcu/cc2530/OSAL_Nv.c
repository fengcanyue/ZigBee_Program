...
static uint8 initNV( void )
{
  osalNvPgHdr_t pgHdr;//8个字节的Page头部
  uint8 oldPg = OSAL_NV_PAGE_NULL;//0
  uint8 findDups = FALSE;//0
  uint8 pg;

  pgRes = OSAL_NV_PAGE_NULL;//0

  for ( pg = OSAL_NV_PAGE_BEG; pg <= OSAL_NV_PAGE_END; pg++ )//循环六个page:121~126
  {
    HalFlashRead(pg, OSAL_NV_PAGE_HDR_OFFSET, (uint8 *)(&pgHdr), OSAL_NV_HDR_SIZE);//参数（flash页码，偏移量，*buf，读取字节数）

    if ( pgHdr.active == OSAL_NV_ERASED_ID )//0xFFFF表示page没有被激活
    {
      if ( pgRes == OSAL_NV_PAGE_NULL )//如果此页没有激活，且此时pgRes为OSAL_NV_PAGE_NULL（0）,则我们不激活此page，而是将此页作为后面压缩的保留页，如果pgRes不为0，即已经有了保留页，则将此page激活，且使此页投入以后使用中
      {
        pgRes = pg;
      }
      else
      {
        setPageUse( pg, TRUE );//使用本页
      }
    }
    // An Xfer from this page was in progress.
    else if ( pgHdr.xfer != OSAL_NV_ERASED_ID )//ffff
    {
      oldPg = pg;
    }
  }
