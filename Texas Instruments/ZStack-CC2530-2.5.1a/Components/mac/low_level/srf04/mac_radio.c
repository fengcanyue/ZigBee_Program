...
MAC_INTERNAL_API uint8 macRadioSetTxPower(uint8 txPower)
{
   ...
   */
  {
    uint8 index = pTable[MAC_RADIO_DEFS_TBL_TXPWR_FIRST_ENTRY] - txPower//txPower是我们自己定义的，要多少DB，就写他妈多少，这里似乎是写了2DB
      + MAC_RADIO_DEFS_TBL_TXPWR_ENTRIES;//index是一个下标，通过这个简单的计算式子，我们就知道所需的发送功率在数组的第几位。把这个“第几位”告诉index
    reqTxPower = pTable[index];//利用了刚刚求出的index，把发送功率的代码告诉reqTxPower
  }
